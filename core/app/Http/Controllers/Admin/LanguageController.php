...
class LanguageController extends Controller
{
    ...
    public function langStore(Request $request)
    {
        $request->validate([
            ...
            'flag'    => ['image', new FileTypeValidate(['jpg', 'jpeg', 'png'])],
        ]);
        ...
        if ($request->hasFile('flag')) {
            try {
                $language->flag = fileUploader($request->flag, getFilePath('flag'), getFileSize('flag'));
            } catch (\Exception $exp) {
                $notify[] = ['error', 'Couldn\'t upload flag'];
