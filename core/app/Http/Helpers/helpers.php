...
function siteCover() {
    return getImage(getFilePath('logoIcon') . '/cover.png');
}
...
function gatewayRedirectUrl($type = false) {
    if (session()->get('THANKS_LINK') || session()->get('WELCOME_LINK') || session()->get('THANKS_GOAL_LINK')) {
        return "home";
    }
    if ($type) {
...
function avatar($image) {
    return $image ? getImage($image) : asset('assets/images/user/avatar.png');
}

function photo($image) {
    return $image ? getImage($image) : asset('assets/images/default.png');
}
...
function checkOwner($pageOwnerId) {
    $authId = @auth()->user()->id;
    if (auth()->check() && $authId === $pageOwnerId) {
        return true;
    }
}
...
function dayNameToDate($dayName) {
    return date('d', strtotime('last ' . $dayName));
}
...
function percent($starting, $target) {
    return ($starting * 100) / $target;
}
